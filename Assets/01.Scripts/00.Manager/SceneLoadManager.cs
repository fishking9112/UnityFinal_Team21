...
public class SceneLoadManager : MonoSingleton<SceneLoadManager>
{
    ...
    public async UniTask LoadScene(string sceneName)
    {
        ...
        {
            Utils.LogError($"해당 이름을 가진 씬은 없습니다 : {sceneName}");
            return;
        }

        if (isSceneLoading)
        {
            Utils.LogError($"이미 씬을 로딩중에 있기 때문에 해당 씬을 로드할 수 없습니다 : {sceneName}");
            return;
        }
        ...
        switch (sceneName)
        {
            case "LoginScene": // 로그인 씬 일 경우
                ...
                await LoadSceneAsync(sceneName); // 로드 씬으로 이동
                ...
                await LoadScene("MenuScene"); // 다 됬으면 메뉴 씬으로 이동
                break;
